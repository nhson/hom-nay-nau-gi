# Spec: Phase 1 — Chay, Bữa 1 món, Vùng miền
...
1. **Chay** — thuộc tính (tag) trên món ăn + bộ lọc "ăn chay hôm nay" áp dụng cho toàn bộ Bữa cơm 3 món hoặc Bữa 1 món, không phải một Nhóm món thứ 4.
2. **Bữa 1 món** — một **Kiểu bữa ăn** mới, song song với "Bữa cơm 3 món" hiện có, kích hoạt chủ động bằng nút riêng (không random tự động).
...
8. As người dùng đang ở chế độ chay VÀ muốn ăn 1 tô cùng lúc, tôi muốn 2 bộ lọc kết hợp được, để chỉ random trong các món Bữa 1 món có tag chay.
...
10. As người dùng đang ở chế độ Bữa 1 món, tôi muốn bấm "đổi món" để random món 1-tô khác, tương tự cơ chế reroll hiện có.
11. As người dùng, tôi muốn quay lại Bữa cơm 3 món bất cứ lúc nào từ Bữa 1 món, để không bị kẹt trong chế độ 1 tô.
12. As người quản lý ngân hàng món ăn, tôi muốn thêm/sửa/xoá món thuộc Bữa 1 món (tên, nguyên liệu, hướng dẫn nấu y như món thường), để tự bổ sung món 1-tô của gia đình mình.
13. As người dùng xem "Danh sách đi chợ" khi đang ở Bữa 1 món, tôi muốn chỉ thấy nguyên liệu của đúng 1 món đó, để đi chợ đúng những gì cần mua.
14. As người dùng đánh dấu "Đã nấu hôm nay" khi đang ở Bữa 1 món, tôi muốn lịch sử ghi nhận đúng đã nấu 1 món đó, để lịch sử phản ánh đúng thực tế.
15. As người dùng xem "7 ngày gần đây", tôi muốn phân biệt được ngày nào là Bữa cơm 3 món, ngày nào là Bữa 1 món.
16. As người dùng, tôi muốn Bữa 1 món KHÔNG bao giờ tự động xuất hiện khi bấm "Random thực đơn" bình thường, để không bị áp đặt món nước khi đang muốn ăn cơm.
...
21. As người dùng mới, tôi muốn ngân hàng món ăn có sẵn 1 lượng món chay và món Bữa 1 món hợp lý ngay từ đầu, để dùng ngay được 2 tính năng mới mà không phải tự thêm từ số 0.
...
- **UI — tab "Món ăn"**: thêm chip lọc "Chay" ngang hàng các chip Nhóm món hiện có (Tất cả/Xào.../Canh/Món mặn); món thuộc Bữa 1 món hiển thị/quản lý trong cùng danh sách (nhóm hiển thị riêng hoặc chip riêng — quyết định cụ thể lúc code).
- **Dữ liệu seed mới**: nghiên cứu và biên soạn danh sách món chay (rà soát 54 món hiện có + thêm món mới nếu cần) và món Bữa 1 món (phở, bún, nui...) theo đúng cấu trúc `Dish` đã mở rộng.
...
- Xem đầy đủ thuật ngữ tại [CONTEXT.md](../CONTEXT.md); quyết định kiến trúc liên quan tại [ADR-0002](adr/0002-meal-type-vs-category.md) (Bữa 1 món là Kiểu bữa ăn riêng, không nhét vào Nhóm món "Canh").
